...
def plot_verts_and_edges(vs, edges, e_labels=None, v_labels=None, show=True, verts=True, edge_colors='k', bbox=None, ax=None):
    ...
    if d == 1:
        ...
        if edges is not None:
            lc = LineCollection(torch.concat(
                [vs[edges], torch.zeros(len(edges), 2, 1)], 2), colors=edge_colors)
    ...
    if v_labels is not None:
        for pos, label in zip(vs, v_labels):
            ax.text(*pos, label, ha='center', va='center',
                    color='green', fontsize=8, zorder=10)
    if not show:
        return ax
    plt.show()


def plot_field(model, ax, dim=2, model_name="", print_steps=1000, cmap="cividis", levels=30, xmin=-2, xmax=2):
    assert dim in (2, 3)
    xi = torch.linspace(xmin, xmax, print_steps)
    yi = torch.linspace(xmin, xmax, print_steps)

    Xi, Yi = torch.meshgrid(xi, yi)
    space = torch.vstack([Xi.flatten(), Yi.flatten()]).T

    out = model(space).detach()
    out = out[:, 0].reshape(Xi.shape)

    if dim == 3:
        ax.plot_surface(Xi, Yi, out, cmap=cmap)
    else:
        ax.contour(Xi, Yi, out, levels=levels, cmap=cmap)
        ax.contourf(Xi, Yi, out, levels=levels, cmap=cmap, alpha=0.2)
        ax.set_aspect("equal")
