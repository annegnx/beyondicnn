...
results_save_path = f"./results/input_dim_widths/"
...
input_dims = [2, 3, 4, 5, 6, 7, 8, 9, 10]
widths_lay = [2, 3, 4, 5, 6, 7, 8, 9, 10]

times_computations = torch.zeros((len(widths_lay), len(input_dims)))
res_convex = torch.zeros((len(widths_lay), len(input_dims)))
res_icnn = torch.zeros((len(widths_lay), len(input_dims)))

for i, w1 in enumerate(widths_lay):
    for j, input_dim in enumerate(input_dims):
        print("input_dim", input_dim, "width", w1)
        convex_nets = torch.zeros(n_test)
        icnn_nets = torch.zeros(n_test)
        sanity_check = torch.zeros(n_test)
        t1 = time()
        for k in trange(n_test):
            f = FeedForwardNet(input_dim=input_dim, widths=[w1, w1]).to(device)
